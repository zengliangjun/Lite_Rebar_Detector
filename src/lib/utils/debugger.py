...
class Debugger(object):
  ...
  def show_img(self, pause = False, imgId = 'default'):
    _img = self.imgs[imgId]
    if _img.shape[0] > 1024 or _img.shape[1] > 1024:
      _img = cv2.resize(_img, (_img.shape[1]// 2, _img.shape[0]// 2))
    cv2.imshow('{}'.format(imgId), _img)
    if pause:
      cv2.waitKey()
  ...
  def show_all_imgs(self, pause=False, time=0):
    if not self.ipynb:
      for i, v in self.imgs.items():
        ...
        if v.shape[0] > 1024 or v.shape[1] > 1024:
  ...
  def save_img(self, imgId='default', path='./cache/debug/'):
    ...
    import time
    cv2.imwrite(path + '{}.png'.format(time.time()), self.imgs[imgId])
